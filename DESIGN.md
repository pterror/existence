...
**Gradients, not binaries.** Nothing in the simulation switches on or off at a threshold. State shapes experience continuously — what's available, what it costs, how the prose reads. Being broke isn't a state you enter at $0. It's a texture that deepens as money tightens, and at every point along that gradient there's *something* you can do, even if it's worse, harder, or more desperate than what was there before. The same principle applies everywhere: energy, hunger, stress, social connection, job standing. The simulation never runs out of content at an extreme — it finds what life looks like there.
