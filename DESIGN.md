...
**Simulation internals are hidden. Player choices are explicit.** "No visible quantification" means the player never sees the simulation's internal accounting — energy levels, NT values, drift rates, the exact cost of a bus ride. Those numbers are implementation details; what the player sees is the experience those numbers produce. But this principle does not extend to replacing player decisions with arbitrary constants or random draws. When the player is making a choice that involves a quantity — how much money to send a friend, how many items to buy, how long to sleep — that quantity belongs to the player, not the game. Build the input. The simulation models consequences; it doesn't make decisions on the player's behalf by substituting a number the player never agreed to. The test: is this number a consequence of the world (hidden), or is it a choice the player is making (explicit)?
