...
- **Sleep architecture** — `sleepCycleBreakdown(minutes)`: variable-length cycles (75/90/100/105 min), deep/REM ratio shifts across cycles. Adenosine clearing scales with deep sleep fraction. NE clearing scales with REM fraction × quality. Emotional processing quality = qualityMult × (0.4 + 0.6 × remFrac). Sleep inertia from cycle phase at wake (0–0.6). Per-character cycle length variation is a TODO (stable biological trait, charRng at chargen).
