...
**Phone battery model is wrong.** Drain is per-check (-1% each time you pick it up), but that's not how batteries work. Screen time is what drains a phone — glancing at the time for 5 seconds is different from scrolling messages for 20 minutes. Battery should drain based on how long you're using it, not how many times you look at it. Also: there's no way to charge. Charging should be a thing you do or forget to do — plugging in at home, at work. The phone dying because you forgot is texture. The phone dying because there's no charger is a bug.
...
**Events fire on the wrong model.** All events currently use probabilistic checks every action — 12% chance if hunger > 65, 6% chance in apartment, 8% chance on street. This is wrong for two reasons. First, body-state events (hunger, exhaustion, lateness) shouldn't be probabilistic and recurring — they should fire on state *transitions*. You discover you're hungry once, when hunger crosses into a new tier. After that, hunger is carried by prose texture (descriptions, focus, what you notice), not by the game re-announcing "you're hungry" on a dice roll. Second, ambient events (pipes, street noise, apartment sounds) shouldn't accumulate endlessly either. Sitting in the apartment for an hour doesn't produce 15 pipe-clank events — you habituate. Ambient sounds become background the longer you're in the same place. Events should be tied to change: arriving somewhere, a state transition, something actually new. The caps (2 per type) were a bandaid that traded repetition for silence. Both are wrong.
