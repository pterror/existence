...
- **SNAP / food stamps / EBT** — a monthly benefit balance separate from cash. Characters with certain economic origins or life events would be enrolled. Changes `buy_groceries` and `buy_cheap_meal` availability: instead of `canAfford(8)`, check cash OR EBT balance. EBT can't buy hot food (so `buy_cheap_meal` stays cash-only). Balance arrives on a monthly schedule like a bill. Enrollment is a backstory parameter — not everyone is on it, and some who qualify aren't enrolled. This is one of the most important $0 pathways and needs to be modeled honestly.
- **Soup kitchen / community meal** — a location (or a scheduled event at an existing location) that provides a free hot meal. Costs time and travel. Available on specific days/hours (many are weekday lunch only, some evenings). Dignity cost varies by character — some people have used these before, some haven't; the first time is different from the tenth. Needs either a new location or a travel path to an existing one.
- **Food bank** — different from a soup kitchen. You go, you get a bag of groceries, you take it home. Stocks the fridge/pantry. Less frequent (weekly or biweekly, appointment-based or walk-in). A separate location. Requires knowing it exists (backstory) or finding out (social network, phone search).
- **Dry pantry / shelf-stable** — most people with an "empty fridge" still have something in the cupboard. A `pantry_food` pool separate from `fridge_food`, stocked when buying groceries (some goes to fridge, some to shelf), depletes separately. Ramen, rice, crackers, a tin of beans. This is cheap to implement and significantly more honest.
...
- **Asking someone** — texting a friend "hey can you spot me" or "can you bring something over." Phone interaction. Either cash transfer (requires the friend to have money and feel close enough) or physical delivery (friend brings food, different texture). Not yet built.
