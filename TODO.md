...
- **Energy drain: 3 pts/hr base, hunger multipliers 1.3×/1.8×** — **RESEARCHED 2026-02-20** (see RESEARCH-CALIBRATION.md §Energy Drain). Flat rate is wrong model shape (circadian profile). Hunger multipliers too aggressive (1.1×/1.3× better grounded). Walking should give net energy bonus. Correct fix: circadian-modulated drain rate + activity-type modifiers. Not yet implemented.
- **NT rate constants table** — **RESEARCHED 2026-02-20 for four mood-primary systems** (see RESEARCH-CALIBRATION.md §NT Rate Constants). Dopamine and NE rates are order-of-magnitude too slow (current ~12–17h half-life; literature ~1–2h acute). NE asymmetry direction possibly inverted. Serotonin approximately correct. GABA low priority. Remaining 23 placeholder systems unresearched.
...
- **Stress creep: 1 pt/hr above 50** — **RESEARCHED 2026-02-20** (see RESEARCH-CALIBRATION.md §Stress Creep). Wrong model shape: no biological mechanism supports constant within-hour escalation. Real mechanism is ruminative re-activation (impaired decay, not autonomous escalation). Proposed fix: remove upward creep; reduce decay rate when `rumination` is high. Cortisol half-life ~80 min at genuine rest. Not yet implemented.
