...
**Event surfacing caps are a bandaid for shallow pools.** Hunger, exhaustion, lateness, and mess events cap at 2 per type, then stop entirely. The caps were added because the events were repeating from a small pool — the same text coming up again, which felt wrong. But capping at 2 trades one problem (repetition) for another (silence). The real fix is deeper event pools and recency tracking, the same way idle thoughts already work (tracking last 4 shown to avoid repeats). Events should keep surfacing as long as the state warrants it — hunger at 90 shouldn't stop being mentioned. But they should never say the same thing twice. And as the state deepens, the events should change character — the first hunger pang is a reminder, the third is the body insisting, the fifth is all you can think about.
