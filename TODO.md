...
- ~~**Asking someone**~~ — **IMPLEMENTED.** `ask_for_help` phone interaction, available when `moneyTier` is `broke` or `scraping`, viewing a friend thread, with no pending reply and 7-day cooldown. Player message varies by mood (heavy/hollow/fraying/flat/okay). Friend response weighted by accumulated warmth sentiment toward that friend (default ~65% chance to help). Help sends $20 via `effect: { type: 'receiveMoney', amount: 20 }` on the pending reply — money arrives with the message. Response prose is flavor-aware (sends_things/dry_humor/warm_quiet/checking_in). Decline is apologetic. Approximation debts: $20 is flat (should vary by relationship depth / friend's finances, which aren't modeled). Physical delivery (friend brings food) not implemented — only cash transfer.
