...
- `sleep_cycle_length` is drawn uniformly from 70–120 min. Blume et al. 2023 (Sleep Health, n=6,064 PSG cycles) found median 96 min. The distribution is right-skewed, not uniform — cycles cluster around 90–100 min. The uniform draw introduces modest bias toward extremes. A truncated normal (mean ~93, SD ~12, clipped 70–120) would be more accurate.
...
- **Age-dependent N3 scaling not modeled.** Van Cauter et al. 2000 (JAMA, n=149): N3 ~19% at age 16–25, ~3–8% at age 36–50 — an ~80% reduction in midlife. A 45-year-old character currently gets the same N3 as a 22-year-old. Mechanically: adenosine clearance, sleep inertia, and emotional processing all depend on N3. Needs character age fed into `cycleFracs()` as a scaling factor on the deep-sleep anchors. REM is stable across 22–48 (Joffe et al.: −0.6% per decade).
...
- **Post-prandial hormonal satiation phase missing.** Ghrelin is suppressed 2–4 hours after a meal independently of stomach volume (CCK, GLP-1, PYY also persist). Currently hunger begins rising immediately as the stomach empties. A post-prandial `hormonal_satiation` variable decaying over 2–4 hours would prevent hunger from rising too fast during gastric emptying. Structural debt — needs a new state variable.
...
- **Cortisol temporal filtering missing.** Cortisol acts on GI motility over hours (slow genomic pathway), not minutes like NE (fast synaptic pathway). The current `gastricSlowFactor` uses instant cortisol equally with NE. Correct model: cortisol contribution should use a rolling average or separate slow-acting inhibition variable. Approximation debt: for typical stress durations in the sim this has modest impact, but is wrong in direction for acute cortisol spikes.
